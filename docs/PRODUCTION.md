...
BoundaryLayer v1.3.3 includes a **production-like local validation profile** while preserving the local lab stack in `docker-compose.yml`. This profile helps you test auth, TLS ingress, rate limits, and observability lockdown on a machine you control. It is not a hosted production SaaS offering.
