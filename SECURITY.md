...
Setting `BOUNDARY_LAYER_PROFILE=production-saas` enables OIDC JWT authentication on lab routes, tenant/membership checks, tenant-scoped lab data paths (PostgreSQL and Redis where applicable), cross-tenant denial with audit evidence, and a fail-closed startup gate. It is not configured in the default local lab. Phase 6 adds staging deployment runbooks, secrets inventory, GitHub Environment setup, container build/deploy dry-run scripts, and `make live-staging-validation-package` for full live validation when `RUN_LIVE_STAGING_CHECKS=true`. Live validation requires real staging secrets via untracked `.env.staging` or GitHub Environment `staging` — never commit tokens. See [docs/STAGING_DEPLOYMENT_RUNBOOK.md](docs/STAGING_DEPLOYMENT_RUNBOOK.md).
