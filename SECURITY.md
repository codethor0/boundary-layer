...
Setting `BOUNDARY_LAYER_PROFILE=production-saas` enables OIDC JWT authentication on lab routes, tenant/membership checks, tenant-scoped lab data paths (PostgreSQL and Redis where applicable), cross-tenant denial with audit evidence, and a fail-closed startup gate. It is not configured in the default local lab.

**Production SaaS score is capped at 6/10 until live staging validation passes.** Dry-runs, scaffolds, and structural checks do not count as live production evidence. Use `make check-live-staging-prereqs` and `make production-saas-evidence-runner` only with real staging secrets (never commit `.env.staging`). See [docs/PRODUCTION_10_10_EVIDENCE_MATRIX.md](docs/PRODUCTION_10_10_EVIDENCE_MATRIX.md).
