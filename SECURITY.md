...
Setting `BOUNDARY_LAYER_PROFILE=production-saas` enables OIDC JWT authentication on lab routes, tenant/membership checks, tenant-scoped lab data paths (PostgreSQL and Redis where applicable), cross-tenant denial with audit evidence, and a fail-closed startup gate. It is not configured in the default local lab. Run `make production-saas-check` to see readiness (expects NOT READY locally), `make production-saas-staging-readiness-check` for staging deploy config validation (expects NOT READY locally), `make production-saas-check-example` and `make production-saas-staging-readiness-example` for mocked structural passes in CI, `make staging-smoke-structural` for staging smoke env validation without remote calls, and `make staging-release-gate` for combined local/structural gates (live checks skipped unless `RUN_LIVE_STAGING_CHECKS=true`). Live validation requires real staging secrets via untracked `.env.staging` or GitHub Environment `staging` — never commit tokens.
