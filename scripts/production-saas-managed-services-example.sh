...
echo "NOTE: mocked managed-service example values only — not live connectivity."
...
.venv/bin/python - <<'PY'
from apps.api.managed_services import evaluate_managed_services_readiness
import os

report = evaluate_managed_services_readiness(dict(os.environ), mocked=True)
